...
public function getAllOrders($timeframe = 'all', $status = 'all') {
        $sql = "SELECT o.*, u.name as customer_name, p.product_name 
    ...
    public function getOrderStatusStats() {
        $sql = "SELECT status, COUNT(*) as count FROM order_or_cart GROUP BY status";
        return $this->db->query($sql)->fetch_all(MYSQLI_ASSOC);
    }

    public function updateOrderStatus($orderId, $newStatus) {
        $stmt = $this->db->prepare("UPDATE order_or_cart SET status = ? WHERE order_cart_id = ?");
